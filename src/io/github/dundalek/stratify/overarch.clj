...
(defn- ->graph [model]
  (let [serialize-id str
        child-parent-map (->> (:relations model)
                              (filter (comp #{:contained-in} :el))
                              (map (fn [{:keys [from to]}]
                                     [(serialize-id from) (serialize-id to)]))
                              (into {}))
        edges (->> (:relations model)
                   (remove (comp #{:contained-in} :el)))
        graph (-> (lg/digraph)
                  (lg/add-nodes* (map (comp serialize-id :id) (:nodes model)))
                  (lg/add-edges* (->> edges
                                      (map (fn [{:keys [from to]}]
                                             [(serialize-id from) (serialize-id to)])))))
        graph (reduce (fn [g node]
                        (let [{:keys [id el] node-name :name} node
                              id (serialize-id id)
                              base-attrs (merge {:Label node-name
                                                 :Category (name el)}
                                                (update-vals (dissoc node :id :el :ct) str))
                              attrs (if-some [parent (child-parent-map id)]
                                      (assoc base-attrs :parent parent)
                                      base-attrs)]
                          (add-attrs g id attrs)))
                      graph
                      (:nodes model))
        graph (reduce (fn [g relation]
                        (let [{:keys [from to el] link-name :name} relation
                              attrs (merge {:Category (name el)
                                            :Label link-name}
                                           (update-vals (dissoc relation :id :el :from :to :name) str))]
                          (add-attrs g [(serialize-id from) (serialize-id to)] attrs)))
                      graph
                      edges)]
    graph))
...
(defn extract-graph [{:keys [source-paths]}]
...
(defn extract-dgml [{:keys [source-paths]}]
  (let [model (read-model! source-paths)]
    (->dgml model)))

(defn extract [{:keys [source-paths output-file]}]
  (let [data (extract-dgml {:source-paths source-paths})]
    (sdgml/write-to-file output-file data)))

(comment
  (def source-paths ["test/resources/overarch/model.edn"])
